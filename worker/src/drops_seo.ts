...
function vsPage(name: string, slug: string, what: string): DropPage {
  return {
    ...
    cat: "vs Bots",
    kind: "versus",
    title: `QuickCatch vs ${name}: free Pokémon drop catcher`,
    desc: `QuickCatch vs ${name}. A free, browser-based way to catch Pokémon and TCG restocks, with no server, no proxies, and no monthly fee.`,
