...
var QC_SEL_URL = "https://wmcp.sh/api/v1/selectors";
var QC_SEL_TTL = 6 * 60 * 60 * 1e3;
var qcSelCache = null;
function qcValidHosts(h) {
  ...
  for (const k in h) {
    if (Array.isArray(h[k])) {
      const arr2 = h[k].filter((s) => typeof s === "string" && s.length < 200).slice(0, 12);
      if (arr2.length) out[k] = arr2;
    }
  }
...
async function qcGetSelectors() {
  ...
  try {
    ...
    if (stored && Date.now() - stored.ts < QC_SEL_TTL) {
      qcSelCache = stored;
      return stored.hosts;
    }
  } catch {
  ...
  try {
    ...
    if (hosts) {
      qcSelCache = { ts: Date.now(), hosts };
      chrome.storage.local.set({ qc_selectors: qcSelCache });
      return hosts;
    }
  } catch {
  }
  try {
    const stored = (await chrome.storage.local.get("qc_selectors")).qc_selectors;
    if (stored) return stored.hosts;
  } catch {
  }
  return null;
}
var QC_W_KEY = "qc_watches";
var QC_ALARM = "qc-poll";
var QC_POLL_MIN = 1;
function qcNorm(u) {
  try {
    const x = new URL(u);
...
async function qcWatches() {
  return (await chrome.storage.local.get(QC_W_KEY))[QC_W_KEY] || {};
}
...
function qcAvailInStock(node, depth) {
  ...
  for (const k in node) {
    ...
    if (v && typeof v === "object") {
      if (Array.isArray(v)) {
        for (const it of v) if (qcAvailInStock(it, depth + 1)) return true;
...
function qcHtmlInStock(html) {
  ...
  for (const b of blocks) {
    ...
    try {
      if (qcAvailInStock(JSON.parse(j), 0)) return true;
    } catch {
    }
  }
...
async function qcPoll() {
  ...
  if (!active.length) {
    chrome.alarms.clear(QC_ALARM);
    return;
  }
  for (const [url, w] of active) {
    if (w.opened && Date.now() - (w.openedAt || 0) < 9e4) continue;
    let html = null;
    try {
      html = await fetch(url, { credentials: "include", cache: "no-store" }).then((r) => r.ok ? r.text() : null);
    } catch {
    }
    if (!html) continue;
  ...
  (async () => {
    try {
      ...
      if (msg?.type === "WATCH_CAUGHT") {
        const w = (await qcWatches())[qcNorm(msg.url)];
        if (w) {
          await qcSaveWatch(msg.url, { ...w, active: false, caught: true, caughtAt: Date.now() });
          await qcEnsureAlarm();
        }
        sendResponse({ ok: true });
        return;
